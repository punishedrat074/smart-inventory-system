...
| Tool                   | Version   | Role                                                                 |
| ---------------------- | --------- | -------------------------------------------------------------------- |
| **PostgreSQL**         | 18.4      | Relational database — ACID guarantees, foreign key integrity         |
| **Neon**               | Cloud     | Hosted PostgreSQL — dev + prod on same service for parity            |
| **Prisma ORM**         | 7.9.1     | Type-safe query builder, schema-as-code, migration management        |
| **@prisma/adapter-pg** | 7.9.1     | Prisma 7 driver adapter — required for Node.js runtime connections   |
| **pg**                 | latest    | Underlying PostgreSQL driver, used by the adapter                    |
| **Prisma Client**      | generated | Auto-generated TypeScript client (singleton in `config/database.ts`) |

### 7.2 Prisma 7 Architecture Change

Prisma 7 introduced two breaking changes from Prisma 5/6:

**1. Connection URL moved out of schema.prisma**

Previously: `url = env("DATABASE_URL")` in the `datasource` block of `schema.prisma`.  
Now: URL is in `server/prisma.config.ts` (for CLI commands) and read from `process.env.DATABASE_URL` at runtime.

```
server/
├── prisma/
│   ├── schema.prisma          ← defines models + generator (no URL)
│   └── migrations/            ← SQL migration files
├── prisma.config.ts           ← NEW: Prisma CLI configuration (database URL, migration path, seed command)
└── src/
    ├── config/
    │   └── database.ts        ← PrismaClient singleton with adapter
    └── generated/
        └── prisma/            ← GENERATED: TypeScript client (gitignored, regenerated by db:generate)
```

**2. Driver adapter required for PrismaClient**

Prisma 7 no longer provides an implicit database connection — you must pass a driver adapter:
...
This makes the database driver choice explicit and enables future support for Edge runtimes (Cloudflare Workers, etc.) by swapping adapters.

### 7.3 Prisma Client Singleton

The singleton pattern prevents connection pool exhaustion during hot-reload in development:
...
- In **development**: `tsx watch` re-imports modules on each save. Without this pattern,
  each hot-reload opens a new connection pool, eventually exhausting Neon's ~100 connection limit.
- In **production**: no hot-reload, so the global store isn't needed — but the client is still created once.

### 7.4 Prisma Scripts (server/package.json)

| Script              | Command                 | When to use                                             |
| ------------------- | ----------------------- | ------------------------------------------------------- |
| `db:generate`       | `prisma generate`       | After any schema change — regenerates TypeScript client |
| `db:migrate`        | `prisma migrate dev`    | During development — creates + applies migration        |
| `db:migrate:deploy` | `prisma migrate deploy` | In CI/production — applies pending migrations only      |
| `db:push`           | `prisma db push`        | Quick schema sync without migration files (prototyping) |
| `db:studio`         | `prisma studio`         | Visual database browser at localhost:5555               |
| `db:seed`           | `tsx prisma/seed.ts`    | Populate demo data (Task 08)                            |
| `db:reset`          | `prisma migrate reset`  | Drop and recreate the database (dev only)               |
...
### 7.8 Neon-Specific Notes

- **SSL:** Neon requires SSL. The connection string format is `postgresql://...?sslmode=require`.
  A deprecation warning from `pg-connection-string` may appear in development logs — this is non-critical
  and will be resolved in pg v9. To suppress: append `?sslmode=verify-full` to `DATABASE_URL`.
- **Connection pooling:** Neon's standard connection string (used here) is direct — no PgBouncer.
  For high-traffic production, consider adding Neon's connection pooler URL as `DIRECT_URL`.
- **Shadow database:** Prisma Migrate needs a shadow database for migration diff calculations.
  Neon supports this on paid plans. For free tier dev, `prisma db push` is an alternative.

### 7.9 Seed Data (for portfolio demo)
